<script lang="ts">
  ...
  let {change, state = $bindable(), onchange}: UpDownButtonsProps = $props();

  function buttonPressed(change: number) {
...
<div class="plus-minus-button-container">
  <button class="plus-button" aria-label="Increase Value" tabindex={-1} onclick={() => buttonPressed(change)}
  ></button>
  <button
    class="minus-button"
    aria-label="Decrease Value"
    tabindex={-1}
    onclick={() => buttonPressed(-change)}
  ></button>
</div>
...
<style>
  ...
  :global([data-theme="light"]) .plus-minus-button-container {
