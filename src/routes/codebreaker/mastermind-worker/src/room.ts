...
export class Room {
  ...
  validateConnection(playerName: string): { valid: boolean; error?: string; code?: number; connectionType?: "firstConnect" | "reconnect" } {
    // check if session is already full (max 2 players)
    if (this.sessions.size >= 2) {
      return { valid: false, code: 503, error: "Session is full. Maximum 2 players allowed." };
    }

    // check if player is already in the session
    if (this.sessions.has(playerName)) {
      return { valid: false, code: 409, error: "Player already connected to this session." };
    }

    // check if player exists in gameState (so the game is running)
    ...
    if (this.gameState.status === GameStatus.Lobby && this.sessions.size < 2) {
      return { valid: true, connectionType: "firstConnect" };
    }

    if (gameStarted && !playerInGameState) {
      return { valid: false, code: 401, error: "Game has already started." };
    }
    ...
    return { valid: false, code: 400, error: "unknown connection type" };
  }
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (!connectionValidation.valid) {
      return new Response(JSON.stringify({ error: connectionValidation.error }), { status: connectionValidation.code || 400 });
    }
  ...
  handleUpdateSettings(msg: any) {
    if (
      msg.numColors < NUM_COLORS_MIN ||
      msg.numColors > NUM_COLORS_MAX ||
      msg.numPositions < NUM_POSITIONS_MIN ||
      msg.numPositions > NUM_POSITIONS_MAX ||
      msg.maxAttempts < MAX_ATTEMPTS_MIN ||
      msg.maxAttempts > MAX_ATTEMPTS_MAX
    ) {
      this.broadcast(
        JSON.stringify({
          type: "chatMessage",
          sender: "System",
          message: `Number of colors must be between ${NUM_COLORS_MIN} and ${NUM_COLORS_MAX}, and number of positions must be between ${NUM_POSITIONS_MIN} and ${NUM_POSITIONS_MAX}, and maximum attempts must be between ${MAX_ATTEMPTS_MIN} and ${MAX_ATTEMPTS_MAX}.`,
        }),
      );
      this.sendState();
      return;
  ...
  async handleSubmitSecret(msg: any, playerName: Player) {
    if (playerName !== this.gameState.activePlayer || playerName !== this.gameState.codeMaker) {
      this.toPlayer(
        playerName,
        JSON.stringify({ type: "chatMessage", sender: "System", message: "Only the code maker can set the secret." }),
      );
      this.sendState();
      return;
    }
    if (
      !Array.isArray(msg.secret) ||
      msg.secret.length !== this.gameState.numPositions ||
      msg.secret.some((color: number) => color <= 0 || color > this.gameState.numColors)
    ) {
  ...
  handleUpdateGuess(msg: any, playerName: Player) {
    if (playerName !== this.gameState.activePlayer || playerName !== this.gameState.codeBreaker) {
      this.toPlayer(
        playerName,
        JSON.stringify({ type: "chatMessage", sender: "System", message: "Only the code breaker can update their guess." }),
      );
      this.sendState();
      return;
    }
  ...
  async handleSubmitGuess(msg: any, playerName: Player) {
    if (playerName !== this.gameState.activePlayer || playerName !== this.gameState.codeBreaker) {
      this.toPlayer(
        playerName,
        JSON.stringify({ type: "chatMessage", sender: "System", message: "Only the code breaker can submit their guess." }),
      );
      this.sendState();
      return;
    }
    if (
      !Array.isArray(msg.guess) ||
      msg.guess.length !== this.gameState.numPositions ||
      msg.guess.some((color: number) => color <= 0 || color > this.gameState.numColors)
    ) {
      this.toPlayer(playerName, JSON.stringify({ type: "chatMessage", sender: "System", message: "Invalid guess." }));
      this.sendState();
      return;
  ...
  handleUpdateFeedback(msg: any, playerName: Player) {
    if (playerName !== this.gameState.activePlayer || playerName !== this.gameState.codeMaker) {
      this.toPlayer(
        playerName,
        JSON.stringify({ type: "chatMessage", sender: "System", message: "Only the code maker can update feedback." }),
      );
      this.sendState();
      return;
    }
    ...
    if (
      !Array.isArray(msg.feedback) ||
      msg.feedback.some((f: FeedbackType) => f !== FeedbackType.Correct && f !== FeedbackType.WrongPosition && f !== FeedbackType.Empty)
    ) {
  ...
  async handleSubmitFeedback(msg: any, playerName: Player) {
    if (playerName !== this.gameState.activePlayer || playerName !== this.gameState.codeMaker) {
      this.toPlayer(
        playerName,
        JSON.stringify({ type: "chatMessage", sender: "System", message: "Only the code maker can submit feedback." }),
      );
      this.sendState();
      return;
    }
    ...
    if (msg.feedback.every((f: FeedbackType) => f === FeedbackType.Correct)) {
      ...
      msg.feedback.some((f: FeedbackType) => f !== FeedbackType.Correct) &&
      this.gameState.guessHistory.length >= this.gameState.maxAttempts
    ) {
      this.gameState.activePlayer = null;
